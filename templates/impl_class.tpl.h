...
// {{ '{{{' }} fake {{ className }}Base
#if 0
//This is a model of our imaginary base class '{{ className }}Base'
...
public:
    ...
    {% if ctor %}
        {% for constructor in constructors.lst %}
            {{ className }}Base({{ defs.arglst(constructor.arguments) }});
        {% endfor %}
    {% endif %}
    ...
    {% else %}
        {# thus this exposed == 'once', the rhs.value is the name to expose to #}
           {# todo only generate the default constructor if that was not already generated by .lst #}
        {{ className }}Base ();
    {% endif %}
protected:
    void registrationHook() = 0;
    // Methods
...
class {{ prefix }}{{ className }} : public ClassMapper{% if hasAttr(extAttrs, 'Events') %}
                                             WithEvents
                                             {% endif %}<{{ prefix }}{{ className }}>, public {{ className }}Base
{
public:
{% if ctor %}
    {% for constructor in constructors.lst %}
             {{ prefix }}{{ className }}( {{ defs.arglst(constructor.arguments) }})
                : {{ className }}Base( {{ defs.argcall(constructor.arguments) }} ) { }
   {% endfor %}
{% else %}
